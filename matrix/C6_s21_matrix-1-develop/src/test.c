...
START_TEST(sub_matrix2) {
  ...
  return_value = s21_sub_matrix(&A, &B, &result);
  if (return_value == 2) {
    ck_assert_int_eq(return_value, ERROR_COUNT);
    s21_remove_matrix(&A);
    s21_remove_matrix(&B);
  }
}
...
START_TEST(transpose) {
  ...
  for (int i = 0; i < result.rows; i++) {
    for (int j = 0; j < result.columns; j++) {
      ck_assert_double_eq_tol(check.matrix[i][j], result.matrix[i][j], 1e-07);
    }
  }
  ck_assert_int_eq(return_value, OK);
  s21_remove_matrix(&A);
...
int main() {
  ...
  tcase_add_test(s21_test, eq_matrix2);
  tcase_add_test(s21_test, sum_matrix);
  tcase_add_test(s21_test, sub_matrix);
  tcase_add_test(s21_test, sub_matrix2);
  tcase_add_test(s21_test, sub_matrix3);
  ...
  tcase_add_test(s21_test, inverse_matrix0);
  tcase_add_test(s21_test, inverse_matrix1);
  tcase_add_test(s21_test, inverse_matrix2);
  tcase_add_test(s21_test, inverse_matrix3);
  tcase_add_test(s21_test, inverse_matrix5);
  tcase_add_test(s21_test, inverse_matrix6);
